...
jest.mock("@/components/CheckoutModal", () => ({
  CheckoutModal: ({
    ...
    return (
      <button data-testid="checkout-confirm" onClick={() => onCryptoPurchase()}>
        Confirm Purchase
      </button>
    );
  },
...
describe("ListingCard", () => {
  ...
  it("shows a truncated artist address", async () => {
    render(
      <ListingCard listing={makeListing({ artist: "GARTIST1234567890" })} />,
    );
    await waitFor(() => {
  ...
  it("calls buy with the listing id when Buy Now is clicked through checkout modal", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("button", { name: /buy now/i }),
      ).toBeInTheDocument(),
    );
  ...
  it("links the image to the listing detail page", async () => {
    ...
    await waitFor(() => {
      ...
      expect(links.some((l) => l.getAttribute("href") === "/listings/3")).toBe(
        true,
      );
    });
  });
  ...
  it("renders a loading state initially", async () => {
    ...
    fetchMetadata.mockReturnValueOnce(
      new Promise((resolve) => {
...
describe("next/image mock", () => {
  const NextImage = jest.requireMock("next/image").default as React.FC<
  ...
  it("does not forward fill to the DOM element", () => {
    const { container } = render(<NextImage src="/test.png" alt="test" fill />);
    const img = container.querySelector("img");
