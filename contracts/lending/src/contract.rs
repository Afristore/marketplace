...
impl LendingContract {
    ...
    pub fn add_collateral(env: Env, position_id: u64, amount: i128) {
        ...
        #[allow(deprecated)]
        env.events()
            .publish((soroban_sdk::symbol_short!("add_col"), position_id), amount);
    }

    pub fn health_factor(env: Env, position_id: u64) -> u32 {
        let position = crate::storage::get_position(&env, position_id);
        if position.declared_price_usd == 0 {
            return 0;
        }

        ((position.collateral_amount * 10_000) / position.declared_price_usd) as u32
    }
}
