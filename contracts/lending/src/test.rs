...
#[test]
fn test_add_collateral_and_health_factor() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    col_admin.mint(&borrower, &50_000_000);
    ...
    // Initial health factor = 10000 bps (100%)
    assert_eq!(client.health_factor(&1), 10000);

    // Borrower adds 50,000,000 collateral
    client.add_collateral(&1, &50_000_000);

    // Contract received the additional collateral
    assert_eq!(col_token.balance(&contract_id), 50_000_000);
    assert_eq!(col_token.balance(&borrower), 0);

    // Position collateral increased to 150,000,000
    let position = env.as_contract(&contract_id, || crate::storage::get_position(&env, 1));
    assert_eq!(position.collateral_amount, 150_000_000);

    // Health factor improved to 15000 bps (150%)
    assert_eq!(client.health_factor(&1), 15000);
}
